                    ...
                    {history.length > 0 ? (
                        history.map((entry) => (
                            <div key={entry.uuid} className="border-b pb-4 last:border-b-0">
                                ...
                                {entry.previousSeverity && entry.newSeverity && entry.previousSeverity !== entry.newSeverity && (
                                    <p><strong>Severity Change:</strong> {entry.previousSeverity} → {entry.newSeverity}</p>
                                )}
                                {entry.previousTitle && entry.newTitle && entry.previousTitle !== entry.newTitle && (
                                    <p><strong>Title Change:</strong> {entry.previousTitle} → {entry.newTitle}</p>
                                )}
                                {entry.previousDescription && entry.newDescription && entry.previousDescription !== entry.newDescription && (
                                    <p><strong>Description Change:</strong> {entry.previousDescription} → {entry.newDescription}</p>
                                )}
                                {entry.changeReason && (
